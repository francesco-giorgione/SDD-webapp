...
router.get('/all/partite/in-vendita', async (req, res) => {
    ...
    for (const user of getAllMilkhubs()) {
        let tmp = await ut.getProdottiProfilo(user.username, ut.getPartiteInVendita, ut.getPartita, true)
        partiteInVendita = partiteInVendita.concat(tmp)
    }
...
router.get('/all/formaggi/in-vendita', async (req, res) => {
    ...
    for (const user of getAllProducers()) {
        let tmp = await ut.getProdottiProfilo(user.username, ut.getFormaggiInVendita, ut.getFormaggio, true)
        formaggiInVendita = formaggiInVendita.concat(tmp)
    }
